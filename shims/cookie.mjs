// ESM shim for `cookie@1.0.2`. The upstream package ships CJS, which Vite's
// dep optimizer occasionally fails to expose as named ESM exports during dev
// HMR — breaking `@supabase/ssr`'s `import { parse, serialize } from "cookie"`
// and cascading into Vue hydration failure.
//
// This shim is a faithful re-implementation aliased via `nuxt.config.ts`
// (`vite.resolve.alias.cookie`). Behavior must match cookie@1.x exactly.
