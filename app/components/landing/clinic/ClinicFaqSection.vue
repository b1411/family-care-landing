...
const faqs = [
  {
    ...
    a: 'Стоимость рассчитывается индивидуально в зависимости от объёма семей и набора модулей. Оставьте заявку — подготовим персональное предложение.',
  },
  {
    q: 'Чем это отличается от МИС?',
    a: 'МИС — это учёт пациентов и меддокументация. UMAI Health — это удержание и сопровождение: маршруты, напоминания, координация, аналитика удержания. Платформа работает автономно, но может интегрироваться с МИС через API.',
  },
  {
  ...
  },
  {
    ...
    a: 'Шифрование AES-256 при хранении и TLS 1.3 при передаче. Row Level Security изолирует данные каждой клиники на уровне СУБД. Журнал действий фиксирует каждую операцию. Построено в соответствии с Законом РК «О персональных данных и их защите».',
  },
  {
  ...
  },
  {
    ...
    a: 'По умолчанию данные хранятся на зашифрованных серверах в ЕС (AES-256). Для клиник с требованием локализации возможно развёртывание на серверах в РК. Ежедневные автоматические резервные копии.',
  },
  {
    q: 'Может ли персонал одной клиники видеть данные другой?',
    a: 'Нет. Row Level Security (RLS) изолирует данные клиник на уровне базы данных — изоляция действует и при прямых API-запросах.',
  },
]
</script>
